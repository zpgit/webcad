...
//
// The store's contract is deliberately narrow: it moves named parts in and out,
 ...
 *
 * Held separately from the parts so that listing never touches a checkpoint:
...
export interface DocumentSummary {
  ...
  /** Total bytes across all parts, for display and for measurement. */
...
export interface DocumentStore {
   ...
   *
   * Either every part lands or the previously stored document is left exactly
   ...
   */
  save(summary: DocumentSummaryInput, parts: DocumentParts): Promise<void>;

  read(documentId: string): Promise<DocumentParts>;

  /** Every stored document, without reading a single checkpoint. */
  list(): Promise<readonly DocumentSummary[]>;

  /**
   * Deletes a document and all of its parts. Clears the last-opened record if
