...
**Migration**: Its three concerns are carried forward, none dropped. Assembly hierarchy and names are now required by `assembly-structure`. Colour at part, instance, and face level is now required by `appearance-attributes`, which also keeps the "reported, not discovered" rule for what it still does not model. The requirement immediately below keeps the explicit-drop rule for everything else STEP carries.

Its scenario "Export writes shape without inventing semantics" is **not** carried forward as written, because it was never true. Measured while building this change's fixtures: the translator fabricates an assembly for any export of more than one body — a root product plus one child per body, with generated names — so the payload of a two-body export has always contained invented structure and invented names. No test caught it, because the round-trip tests asserted geometry fidelity rather than the entity census. `assembly-structure` restates the intent with the mechanism attached: the assembly mode is set by this system on every export rather than inherited from the library, and a flat session exporting flat is something the export does rather than something it hopes for.
