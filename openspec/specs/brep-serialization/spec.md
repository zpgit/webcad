# brep-serialization Specification

## Purpose

Defines the one form in which exact geometry may leave the kernel, and the
discipline that stops it from becoming a second representation.

Every other capability rests on `geometry-kernel`'s rule that canonical B-Rep
never crosses into JavaScript. Persistence needs bytes to store, which is the
only reason that rule has an exception at all — so the exception is drawn as
narrowly as it can be. The payload is produced by OCCT's own binary shape
serialization, it is opaque, and no API decodes it. A caller may store it,
measure it, hash it, and hand it back; parsing it would create exactly the
outside-the-kernel geometry the boundary exists to prevent.

Exactness is the other constraint, and it is why round-trip fidelity is
specified in terms of analytic surface type rather than tolerance alone: a
cylinder that returned as a spline approximation of itself would satisfy any
volume check while destroying the claim the whole system is built on.
Restoration is all-or-nothing for a related reason — a partially restored
payload would leave handles to geometry no document describes.

What this capability does not do is interpret or negotiate its own payload. The
encoding and the OCCT build that wrote it are reported; deciding what to do
about a difference belongs to `native-document`, which has a manifest to record
it in.
