# step-translation Specification

## Purpose

Defines the boundary where STEP becomes canonical geometry and canonical geometry
becomes STEP, and the rules that boundary imposes on everything downstream of it.

The governing constraint is that translation is a boundary concern and must not
leak into the application (note §11). Everything a STEP file carries that this
system does not model - entities, product structure, attribute records - stops
here. What crosses is handles, counts, and scalars, exactly as for geometry
created in this kernel. An imported body is an ordinary body: nothing marks it,
nothing restricts it, and the only thing that remembers where it came from is the
document's provenance record.

Three consequences shape the requirements below.

STEP bytes are a **foreign** payload. Unlike a checkpoint, which this kernel
wrote and a caller can only hand back, a `.step` file comes off a user's disk and
nothing about it may be assumed. That difference is in what gets validated, not
in how the bytes travel.

Units are resolved **here and only here**. The reader converts a file's declared
unit into the working unit as part of the transfer, so the document, the modeling
operations, and the viewport can all assume one unit. Both units are reported,
because a conversion nobody can see is indistinguishable from a fidelity loss.

The kernel's translators **modify shapes by default** - the reader repairs
geometry, the writer reorients faces - and a repair is indistinguishable in the
result from a translation loss. So processing is a caller-controlled setting that
reports what it did, rather than a library default nobody stated.
